...
public class EmployeesServiceMapsImpl implements EmployeesService, Serializable {
	/**
	 * 
	 */
	...
	static private ReentrantReadWriteLock lock;
	transient private Lock readLock;
	transient private Lock writeLock;

	private static String filename = "employees.data";
	
	static public EmployeesServiceMapsImpl getEmployeesService() {
		EmployeesServiceMapsImpl resObj = null;
		try(ObjectInputStream in = new ObjectInputStream(new FileInputStream(filename))) {
			resObj = (EmployeesServiceMapsImpl) in.readObject();
			resObj.setLocks();
			
			
		} catch (Exception e) {
			System.out.println("Data was not restored " + e.getMessage());
			resObj = new EmployeesServiceMapsImpl();
		}
		return resObj;
	}
	...
	private EmployeesServiceMapsImpl(){
		setLocks();
	}
